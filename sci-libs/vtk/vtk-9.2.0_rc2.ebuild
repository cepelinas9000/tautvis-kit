...
# the rc suffix have '.' as seperator

if [ -n "$DL_PV" ]; then
    DL_PV="$(ver_cut 1-3).$DL_PV"
else
    DL_PV="$(ver_cut 1-3)"
fi
...
SRC_URI="
	https://www.vtk.org/files/release/${MY_PV}/VTK-${DL_PV}.tar.gz
	https://www.vtk.org/files/release/${MY_PV}/VTKData-${DL_PV}.tar.gz
	https://www.vtk.org/files/release/${MY_PV}/VTKDataFiles-${DL_PV}.tar.gz
	doc? ( https://www.vtk.org/files/release/${MY_PV}/vtkDocHtml-${DL_PV}.tar.gz )
	examples? (
		https://www.vtk.org/files/release/${MY_PV}/VTKLargeData-${DL_PV}.tar.gz
		https://www.vtk.org/files/release/${MY_PV}/VTKLargeDataFiles-${DL_PV}.tar.gz
	)
	test? (
		https://www.vtk.org/files/release/${MY_PV}/VTKLargeData-${DL_PV}.tar.gz
		https://www.vtk.org/files/release/${MY_PV}/VTKLargeDataFiles-${DL_PV}.tar.gz
...
post_src_unpack() {
    ...
    if [ ! -d "${S}" ]; then
        ORG_SRC=`find "${WORKDIR}"/ -mindepth 1 -maxdepth 1 -type d`
        mv "${ORG_SRC}" "${S}"
        popd
    fi
...
src_prepare() {
	...
	local -a DROPS=( doubleconversion eigen expat freetype hdf5 jpeg jsoncpp
		libxml2 lz4 lzma netcdf ogg png pugixml sqlite theora tiff utf8
		zlib )
